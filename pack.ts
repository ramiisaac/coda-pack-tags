...
pack.addFormula({
  name: "ProcessText",
  description: "Extracts meaningful tags from text by filtering out common words and noise. Returns unique tags separated by commas.",
  parameters: [
    coda.makeParameter({
    ...
    coda.makeParameter({
      ...
      description: "Categories of words to exclude. Valid options: numbers, prepositions, pronouns, conjunctions, auxiliaryVerbs, articles, negations, determiners, adverbs, miscellaneous.",
      optional: true,
    }),
    coda.makeParameter({
      type: coda.ParameterType.StringArray,
...
pack.addFormula({
  name: "ExtractTags",
  description: "Extracts meaningful tags from text using intelligent defaults to filter out common stop words (articles, prepositions, pronouns, etc.). This is a simplified version of ProcessText with sensible defaults.",
  parameters: [
    coda.makeParameter({
      type: coda.ParameterType.String,
      name: "text",
